...
def getsymbol(statement):
    result=[] 
    if(statement.symbol[0]!=""):
        result.append(statement.symbol[0])
    else:
        for char in statement.subexpressions:
            result=result+getsymbol(char)
    result=set(result)
    result=list(result)
    return result

def buildmodel(knowledge_base):   
    count=0
    symbol_to_remove=[]
    model={}
    for i in knowledge_base.subexpressions:
        if(i.connective[0]!="iff" and i.connective[0]!="and" and i.connective[0]!="xor" and i.connective[0]!="or" and i.connective[0]!="if"): 
            if(i.symbol[0]==""): 
                symbol_to_remove.append(i.subexpressions[0].symbol[0])
                model[i.subexpressions[0].symbol[0]]=False
            else: 
                symbol_to_remove.append(i.symbol[0])
                model[i.symbol[0]]=True
    symbol_to_remove=list(set(symbol_to_remove))
    return symbol_to_remove,model

def TTEntails(knowledge_base,statement):
    s1=getsymbol(knowledge_base)
    s2=getsymbol(statement)
    s=s1+s2
    s=set(s)
    s=list(s)  
    smr,model=buildmodel(knowledge_base) 
    for i in smr:
        s.remove(i)  
    return TTCheckall(knowledge_base,statement,s,model,0)
 
def pl_true(sentence,model):
    if(sentence.symbol[0]!=""):
        return model[sentence.symbol[0]]
    elif(sentence.connective[0]=="and"):
        for c in sentence.subexpressions:
            if(pl_true(c,model)==False):
                return False
        return True
    elif(sentence.connective[0]=="or"):
        for c in sentence.subexpressions:
            if(pl_true(c,model)==True):
                return True
        return False
    elif(sentence.connective[0]=="if"):
        left=sentence.subexpressions[0]
        right=sentence.subexpressions[1]
        leftbool=pl_true(left,model)
        rightbool=pl_true(right,model)
...
def TTCheckall(knowledge_base,statement,sym,model,count):   
    if(len(sym)==0):  
        ...
        model1[p]=True
        model2[p]=False 
        a=TTCheckall(knowledge_base,statement,rest,model1,count+1) 
        b=TTCheckall(knowledge_base,statement,rest,model2,count+1)
        return a and b

def check_true_false(knowledge_base, statement):
	result1=TTEntails(knowledge_base, statement)
	negation = logical_expression()
	negation.connective[0] = 'not'
	negation.subexpressions.append(statement)
	result2 = TTEntails(knowledge_base, negation)
